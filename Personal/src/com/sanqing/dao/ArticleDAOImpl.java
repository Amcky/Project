...
public class ArticleDAOImpl extends HibernateDaoSupport implements ArticleDAO {
	...
	public int queryUserAllCount(String username) {
		List find = this.getHibernateTemplate().find("select count(*) from Article art where art.username = ?",
				username);
	...
	public List<Article> queryAllBySearch(final Page page,final String reg) {
		return this.getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				Query query = session.createQuery("select art from Article art  where art.content like ?");
	...
	public void delete(int id,String name) {
		List<Article> list=queryUserAll(name);
